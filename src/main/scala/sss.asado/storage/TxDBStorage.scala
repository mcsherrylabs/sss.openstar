...
object TxDBStorage extends Logging {
  ...
  def confirm(txId: Array[Byte], height: Long, id: Long)(implicit db:Db): Unit = {
    Try {
      ...
      val rowsUpdated = blcokTable.update("confirm = confirm + 1", s"id = $id")
...
class TxDBStorage(tableName: String)(implicit db:Db) extends Storage[TxId, SignedTx] {
  ...
  db.executeSql (s"CREATE TABLE IF NOT EXISTS $tableName (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1), txid BLOB,  entry BLOB, confirm INT DEFAULT 0)")
