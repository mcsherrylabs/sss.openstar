...
import ledger.{TxIndexLen, TxInput}
...
object TxInputSerializer extends Serializer[TxInput]{
  ...
  override def toBytes(t: TxInput): Array[Byte] = {
    ...
    val sigBytes = ContractSerializer.toBytes(t.sig)
  ...
  override def fromBytes(b: Array[Byte]): TxInput = {
    ...
    val dencumbrance = ContractSerializer.fromBytes[Decumbrance](dencumbranceBytes)
