...
class Block(val height: Long)(implicit db:Db) extends Logging {
  ...
  db.executeSql (s"CREATE TABLE IF NOT EXISTS $tableName (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1, INCREMENT BY 1), txid VARCHAR(64) NOT NULL, entry BLOB, confirm INT DEFAULT 0, UNIQUE(txid))")
