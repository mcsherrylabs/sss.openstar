...
import sss.openstar.eventbus.{EventPublish, MessageInfo}
...
object MessageEventBus {
  ...
  case class UnsubscribedEvent[T <: OpenstarEvent](event: T) extends Unsubscribed
...
class MessageEventBus (decoder: Byte => Option[MessageInfo], loggingSuppressedClasses: Seq[Class[_]] = Seq.empty)(
  ...
  override def publish[T <: OpenstarEvent: ClassTag](event: T): Unit = {
