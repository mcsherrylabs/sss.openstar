...
object BlockChainLedger {
  ...
  case class NewBlockId(chainId: GlobalChainIdMask, newId: BlockId) extends OpenstarEvent
...
class BlockChainLedger(block: Block, ledgers: Ledgers) extends Logging {
  ...
  private def commit(entries: Seq[BlockTx]): Seq[OpenstarEvent] = {
    entries
  ...
  /**
    ...
    * @return
    */
  def apply(stx: LedgerItem): Try[(BlockChainTx, Seq[OpenstarEvent])] = block.inTransaction[(BlockChainTx, Seq[OpenstarEvent])] {
  ...
  def apply(blockTx: BlockTx): Try[Seq[OpenstarEvent]] = block.inTransaction[Seq[OpenstarEvent]] {
  ...
  def validate(blockTx: BlockTx): Try[Seq[OpenstarEvent]] = block.validateTx(apply(blockTx).get)

  def validate(stx: LedgerItem): Try[(BlockChainTx, Seq[OpenstarEvent])] = block.validateTx(apply(stx).get)
