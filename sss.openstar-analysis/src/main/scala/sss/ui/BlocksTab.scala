...
class BlocksTab(clientNode: ClientNode) extends VerticalLayout {
  ...
  private val panel = new Panel("Openstar Block")
  ...
  def update(blockHeight:Long) = {
    ...
    clientNode.bc.blockHeaderOpt(blockHeight) match {
      ...
      case Some(header) =>
        ...
        panel.setCaption(s"Openstar Block $blockHeight")
        numInBlock.setValue(header.numTxs.toString)
        if(Analysis.isCheckpoint(blockHeight)) println("This is a checkpoint block")
        if(Analysis.isCheckpoint(blockHeight) || Analysis.isAnalysed(blockHeight)) {
